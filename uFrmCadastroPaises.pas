...
type
  TFrmCadastroPaises = class(TFrmCadastroPai)
    ...
    edt_Ddi: TEdit;
    edt_Sigla: TEdit;
    procedure edt_NomePaisExit(Sender: TObject);
  private
...
procedure TFrmCadastroPaises.edt_NomePaisExit(Sender: TObject);
begin
  inherited;
  ...
  begin
    showmessage('Pais :' +self.edt_NomePais.text+ ' já cadastrado! ');
...
begin
 if length(self.edt_NomePais.Text) = 0 then
  ...
  if ehObrigatorio(self.edt_NomePais.text, '*') and (length(self.edt_NomePais.Text) = 0 )then
  begin
  ...
  else if ehObrigatorio(self.edt_Moeda.text, '*') and (length(self.edt_Moeda.Text) = 0) then
  begin
    showmessage('CAMPO DDI É OBRIGATÓRIO!');
    self.edt_Moeda.SetFocus;
  end
  else
  begin
    ...
    begin
        ...
        umaCtrlPaises.Salvar(umPais.clone);
